...
export const deleteUserHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    if (result.success && result.meta.changes > 0) {
      ...
      return c.json(GeneralServerErrorSchema.parse({ message: 'Failed to delete user due to a database error.' }), 500);
    }
    ...
    //   return c.json(UserDeleteFailedErrorSchema.parse({ message: 'Cannot delete user. User is referenced elsewhere.' }), 409); // Conflict
    // }
