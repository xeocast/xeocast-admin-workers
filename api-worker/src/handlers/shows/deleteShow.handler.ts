...
export const deleteShowHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    if (dependentEpisodes) {
      return c.json(ShowDeleteFailedErrorSchema.parse({ success: false, message: 'Cannot delete show: It is referenced by existing episodes.' }), 400);
    }
    ...
    if (dependentSeries) {
      return c.json(ShowDeleteFailedErrorSchema.parse({ success: false, message: 'Cannot delete show: It is referenced by existing series.' }), 400);
    }
    ...
    if (dependentYouTubeChannels) {
        return c.json(ShowDeleteFailedErrorSchema.parse({ success: false, message: 'Cannot delete show: It is referenced by existing YouTube channels.' }), 400);
    }
    ...
    if (result.success && result.meta.changes > 0) {
      return c.json(ShowDeleteResponseSchema.parse({ success: true, message: 'Show deleted successfully.' }), 200);
    } else if (result.success && result.meta.changes === 0) {
        // This case should ideally be caught by the existence check, but as a safeguard:
        return c.json(ShowNotFoundErrorSchema.parse({ success: false, message: 'Show not found or already deleted.' }), 404);
    }else {
      console.error('Failed to delete show, D1 result:', result);
      return c.json(ShowDeleteFailedErrorSchema.parse({ success: false, message: 'Failed to delete show.' }), 500);
    }
    ...
    return c.json(ShowDeleteFailedErrorSchema.parse({ success: false, message: 'Failed to delete show due to an unexpected error.' }), 500);
  }
};
